...
def mergeX(positive=1):
    ...
    if positive == 1:
        CSV = "LBP_CSV/Positive/"
    else:
        CSV = "LBP_CSV/Negative/"
            ...
            if rows > 1 and cols > 1:
                ...
                for j in range(0, maxRows):
                    for i in range(0, maxCols):
                        ...
                        histG = LBP.getHistogramOfLBP(roi)
                ...
                for dX in range(0, 3):
                    for dY in range(0, 3):
                        ...
                        if classifier.predict(histGE):
                            ...
                            cv2.imwrite("LBP_Img/"+"ID"+str(ID)+str(j*1000)+str(i*100)+str(dY*10)+str(dX)+"Foi"+".jpg",roi)
...
if len(sys.argv) <= 1:
    ...
    if sys.argv[1] == '-c':
        ...
        if sys.argv[2] == 'rbf':
            FILE_NAME = 'Model/modelLBPrbf.sav'
            FILE_NAME_SCALAR = 'Model/scalarLBPrbf.sav'
            X_train, X_test, y_train, y_test, y_pred, classifier, cm, standardScaler = ml.svm(X, y, 'rbf')
        if sys.argv[2] == 'rf':
            FILE_NAME = 'Model/modelLBPRF.sav'
            FILE_NAME_SCALAR = 'Model/scalarLBPRF.sav'
            X_train,X_test,y_train,y_test,y_pred,classifier,cm,standardScaler = ml.randomF(X, y, 100)
        if sys.argv[2] == 'linear':
            FILE_NAME = 'Model/modelLBPLinear.sav'
            FILE_NAME_SCALAR = 'Model/scalarLBPLinear.sav'
